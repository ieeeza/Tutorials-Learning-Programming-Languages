<script setup>
import PropiedadesComputadas from "./components/PropiedadesComputadas.vue";
import PropiedadesAssistidas from "./components/PropiedadesAssistidas.vue";
import RenderizacaoCondicional from "./components/RenderizacaoCondicional.vue";
import RenderizacaoListas from "./components/RenderizacaoListas.vue";
import Slots from "./components/Slots.vue";

const cachorrosDoMano = [
   {nome: "toto", idade: 2},
   {nome: "laika", idade: 4},
   {nome: "flor", idade: 6},
]
</script>

<template>
   <Slots>
      <template v-slot:icone>
         <svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-abacus"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M5 3v18" /><path d="M19 21v-18" /><path d="M5 7h14" /><path d="M5 15h14" /><path d="M8 13v4" /><path d="M11 13v4" /><path d="M16 13v4" /><path d="M14 5v4" /><path d="M11 5v4" /><path d="M8 5v4" /><path d="M3 21h18" /></svg>
      </template>
      <template v-slot:texto>
         HOME
      </template>
   </Slots>
   <RenderizacaoListas nome="jose" :cachorros="[]"/>
   <RenderizacaoCondicional nome="César" :qtd-cachorros="1"/>
   <PropiedadesAssistidas />
   <PropiedadesComputadas />
</template>
